...
void loop() {
  if (loop_trigger == 1){ // FAST LOOP (1kHZ)
      ...
      V_meas = analogRead(A1)*4.096/1000;
      V_pd = analogRead(A3)*4.096/(1000*0.3708);
  ...
  if (int_count == 1000) { // SLOW LOOP (1Hz)
    ...
    switch (state_num) { // STATE MACHINE (see diagram)
      ...
      case 1:{ //Discharge state (-250mA and no LEDs)
         //current_ref = -250;
         if (pwm_out < 0.99) { // While not at minimum volts, stay here
    ...
    dataString = String(state_num) + "," + String(V_meas) + "," + String(V_pd) + "," + String(current_measure) + "," + String(pwm_out); //build a datastring for the CSV file
